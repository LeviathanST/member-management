...
<body>
    ...
    <div class="container">
        ...
        <div class="guild-list-container">
            ...
            <div class="add-guild">
                <button onclick="addGuild()">Add crew</button>
            </div>
        ...
        <script>
            function renderGuildList() {
                ...
                guilds.forEach(guild => {
                    ...
                    guildInfo.innerHTML = `
                        <p><strong>Name:</strong> ${guild.name}</p>
                        <p><strong>Student Code:</strong> ${guild.studentCode}</p>
                    `;
    
                    const guildActions = document.createElement("div");
                    guildActions.classList.add("guild-actions");
    
                    const banButton = document.createElement("button");
                    banButton.textContent = "Ban";
                    banButton.classList.add("btn-ban");
                    banButton.onclick = () => banGuild(guild.studentCode);

                    const editButton = document.createElement("button");
                    editButton.textContent = "Edit";
                    editButton.classList.add("btn-edit");
                    editButton.onclick = () => editGuild(guild.studentCode);

                    const deleteButton = document.createElement("button");
                    deleteButton.textContent = "Delete";
                    deleteButton.classList.add("btn-delete");
                    deleteButton.onclick = () => deleteGuild(guild.studentCode);
                    guildActions.appendChild(banButton);
                    guildActions.appendChild(editButton);
                    guildActions.appendChild(deleteButton);
                    guildItem.appendChild(guildInfo);
                    guildItem.appendChild(guildActions);
                    guildListContainer.appendChild(guildItem);
                });
            }
            function deleteGuild(studentCode) {
                if (confirm(`Are you sure you want to delete guild with student code ${studentCode}?`)) {
                    alert(`Guild with student code ${studentCode} has been deleted.` );
                    const index = guilds.findIndex(guild => guild.studentCode === studentCode);
                    if (index !== -1) {
