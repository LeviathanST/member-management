...
public class UserGuildRoleRepository {
	...
	public static List<UserGuildRole> getAllByGuildId(String guild, int guild_id) throws SQLException,
			NotFoundException, NullPointerException, IOException, ClassNotFoundException {
		try (Connection con = Database.connection()) {
			String query = """
						...
						WHERE gr.guild_id = ?
					""";
			List<UserGuildRole> list = new ArrayList<>();
			PreparedStatement stmt = con.prepareStatement(query);
			stmt.setInt(1, guild_id);

			ResultSet rs = stmt.executeQuery();
			if (rs.next()) {
				list.add(new UserGuildRole(guild, rs.getString("name"), rs.getString("role")));
				while (rs.next()) {
					list.add(new UserGuildRole(guild, rs.getString("name"),
							rs.getString("role")));
				}
			} else {
