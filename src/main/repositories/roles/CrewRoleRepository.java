...
public class CrewRoleRepository {
	...
	public static List<CrewRole> getAllByCrewId(int crewId)
		...
		try (Connection con = Database.connection()) {
			...
			while (rs.next()) {
				list.add(new CrewRole(crewId, rs.getString("name")));
			}
	...
	public static List<CrewRole> getAllByAccountId(String account_id)
		...
		try (Connection con = Database.connection()) {
			...
			while (rs.next()) {
				CrewRole cr = new CrewRole(rs.getInt("id"), rs.getString("name"));
				list.add(cr);
			}
	...
	public static int getIdsByCrewId(int crew_id)
			throws SQLException, NotFoundException, IOException, ClassNotFoundException {
		try (Connection con = Database.connection()) {
