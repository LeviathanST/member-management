...
public class CrewPermission {
	...
	public CrewPermission(int id, String account_id) {
		this.id = id;
	...
	public static List<CrewPermission> getAllByCrewRoleId(Connection con, int crewRoleId)
		...
		String query = "SELECT id, account_id FROM user_crew_role WHERE crew_role_id = ?";
		...
		while (rs.next()) {
			list.add(new CrewPermission(rs.getInt("id"), rs.getString("account_id")));
		}
