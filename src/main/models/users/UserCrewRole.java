...
public class UserCrewRole {
	...
	public static List<Integer> getCrewRoleIdByAccountId(Connection con, String account_id)
			throws SQLException, NotFoundException {
		String query = """
	...
	public static void deleteCrewMember(Connection con, String accountId, int crewId)
		...
		textIO.getTextTerminal().println(String.valueOf(crewId));
		String query = """
			DELETE ucr
	...
	public static List<UserCrewRoleDto> getAllByCrewId(Connection con, String crew, int crewId) throws SQLException, IndexOutOfBoundsException, NotFoundException {
		String query = """
					...
					WHERE cr.crew_id = ?
				""";
		List<UserCrewRoleDto> list = new ArrayList<>();
		PreparedStatement stmt = con.prepareStatement(query);
		...
		if (rs.next()) {
			list.add(new UserCrewRoleDto(crew, rs.getString("name"), rs.getString("role")));
			while (rs.next()) {
				list.add(new UserCrewRoleDto(crew, rs.getString("name"), rs.getString("role")));
			}
		}
